...
inline static int final_cond(int _out_0_x0,int _out_1_x0) {
  switch (_out_0_x0) {
  case 1:
    switch (_out_1_x0) {
    case 1:
...
static const int out_0_x0_f = 0 ;
static const int out_1_x0_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:X0=%d; 1:X0=%d;\n",c,show ? '*' : ':',(int)o[out_0_x0_f],(int)o[out_1_x0_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+dmb.sys, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+dmb.sys, check_globals failed");
  }
...
"str %w[x2],[%[x3]]\n"
"#END _litmus_P0\n"
:[x0] "=&r" (_a->out_0_x0[_i]),[x2] "=&r" (trashed_x2)
...
"str %w[x2],[%[x3]]\n"
"#END _litmus_P1\n"
:[x0] "=&r" (_a->out_1_x0[_i]),[x2] "=&r" (trashed_x2)
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_x0 = malloc_check(size_of_test*sizeof(*(_a->out_0_x0)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_x0_i = ctx.out_0_x0[_i];
      int _out_1_x0_i = ctx.out_1_x0[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x0_i,_out_1_x0_i));
      o[out_0_x0_f] = _out_0_x0_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+dmb.sys Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:X0=1 /\\ 1:X0=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=c913e27eb60f622be0f95306bf58afc1\n");
  fprintf(out,"Cycle=Rfe DMB.SYdRW Rfe DMB.SYdRW\n");
  fprintf(out,"Generator=diycross7 (version 7.54+01(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=DMB.SYdRW Rfe DMB.SYdRW Rfe\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+dmb.sys %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+dmb.sys %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+dmb.sys: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
