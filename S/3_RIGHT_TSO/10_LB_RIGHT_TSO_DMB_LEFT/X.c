...
inline static int final_cond(int _out_1_x0,int _x) {
  switch (_out_1_x0) {
  case 1:
    switch (_x) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:X0=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x0_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("S+dmb.sys, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+dmb.sys, check_globals failed");
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+dmb.sys, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x0_i,_x_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+dmb.sys Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ 1:X0=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=c2abe86986acac7dd650f566d671d7de\n");
  fprintf(out,"Cycle=Rfe DMB.SYdRW Wse DMB.SYdWW\n");
  fprintf(out,"Generator=diycross7 (version 7.54+01(dev))\n");
  fprintf(out,"Com=Rf Ws\n");
  fprintf(out,"Orig=DMB.SYdWW Rfe DMB.SYdRW Wse\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation S+dmb.sys %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time S+dmb.sys %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+dmb.sys: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
