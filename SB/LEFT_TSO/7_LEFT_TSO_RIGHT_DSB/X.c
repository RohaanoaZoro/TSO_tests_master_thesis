...
inline static int final_cond(int _out_0_x2,int _out_1_x2) {
  switch (_out_0_x2) {
  case 0:
    switch (_out_1_x2) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:X2=%d; 1:X2=%d;\n",c,show ? '*' : ':',(int)o[out_0_x2_f],(int)o[out_1_x2_f]);
}
...
"ldr %w[x2],[%[x3]]\n"
"#END _litmus_P0\n"
:[x2] "=&r" (_a->out_0_x2[_i]),[x0] "=&r" (trashed_x0)
...
"dsb ld\n"
"#END _litmus_P1\n"
:[x2] "=&r" (_a->out_1_x2[_i]),[x0] "=&r" (trashed_x0)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x2_i,_out_1_x2_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:X2=0 /\\ 1:X2=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=233b57e31ddd1edca51a34199fee8a91\n");
